...
const Input: React.FC<BaseInputProps> = (props) => {
  ...
  const classes = classNames('react-input-wrapper', {
  ...
  return (
    <div className={classes} style={style}>
      {prepend && <div className="react-input-group-prepend">{prepend}</div>}
      {icon && <div className="icon-wrapper"><Icon icon={icon} title={`title-${icon}`}/></div>}
      <input 
        className="react-input-inner"
        disabled={disabled}
        {...restProps}
      />
      {append && <div className="react-input-group-append">{append}</div>}
    </div>
  )
}
